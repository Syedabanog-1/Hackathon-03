...
async def get_exercise(request: ExerciseRequest):
    logger.info(fFetching exercise for user {request.user_id}, level={request.level})
...
async def submit_exercise(request: SubmitRequest):
    logger.info(fSubmission from user {request.user_id} for exercise {request.exercise_id})
    passed = True
    score = 85
