...
def analyze_python_code(code: str) -> dict:
    ...
    try:
        ...
        issues.append(ReviewIssue(line=e.lineno or 0, severity="error", message=f"Syntax error: {e.msg}", suggestion="Fix the syntax error before proceeding"))
        score -= 30

    lines = code.split("\n")
    for i, line in enumerate(lines, 1):
        if len(line) > 79:
            issues.append(ReviewIssue(line=i, severity="warning", message=f"Line {i} exceeds 79 characters (PEP 8)", suggestion="Break long lines for readability"))
            score -= 2
...
async def review_code(request: CodeReviewRequest):
    logger.info(f"Reviewing code for user {request.user_id}, language={request.language}")
    if request.language == "python":
        analysis = analyze_python_code(request.code)
    else:
        analysis = {"score": 75, "issues": [], "strengths": ["Code submitted"], "improvements": [f"Static analysis for {request.language} not yet available"]}
