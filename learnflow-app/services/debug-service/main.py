...
ERROR_PATTERNS = {
    rNameError: name '(.+)' is not defined: {
    ...
    rTypeError: unsupported operand type\(s\) for (.+): '(.+)' and '(.+)': {
    ...
    rIndexError: list index out of range: {
...
async def debug_code(request: DebugRequest):
    logger.info(fDebugging for user {request.user_id}: {request.error[:80]})
